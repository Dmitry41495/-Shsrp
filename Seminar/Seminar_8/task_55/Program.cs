...
int[,] СhangeСolumnsToRows(int[,] mtrx)
{
    int temp;
    int lastLine = mtrx.GetLength(0);
    int lastСolumn = mtrx.GetLength(1);

    if (lastLine == lastСolumn)
    {
        int begin=0;
        for(int i=0; i<lastLine; i++)
        {
            
            for(int j = begin; j<lastСolumn; j++)
            {
                 temp = mtrx[i, j];
                mtrx[i, j] = mtrx[j, i];
                mtrx[j, i] = temp;
    ...
    {
        System.Console.WriteLine("Количество строк не совпадает с количеством столбцов, Замена невозможна");
    }
    return mtrx;
}